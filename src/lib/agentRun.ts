...
- build_panel: create a chart/stat/table panel the user can pin to their dashboard. Use when they want to "make"/"add"/"pin" a panel. Panelable fields include: project, stage, owner, source, location, needs_help, score, vol_24h, market_cap, fees_24h, price_change_24h, token, contract_address, submitted_at, one_liner, low_effort, needs_review, outreach_types, last_outreach_type, last_outreach_at, last_contact, contact_count, has_token. So a "last contact date" or "outreach type" column IS supported — add it to columns/filters. Don't claim a field isn't supported unless it's genuinely not in this list; if it isn't, say which specific field and offer the closest available one.
